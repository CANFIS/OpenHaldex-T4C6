...
void init_EEPROM()
{
  ...
  else
  {
    ...
    in_standalone_mode = EEPROM.read(EEPROM_ADDRESS_STANDALONE); // Always false, kept for compatibility
    ...
    // Enable mode_override for modes: FWD/5050/7525/CUSTOM. (?)
...
bool update_EEPROM(void *params)
{
  ...
  DEBUG("Software: %d.%d, Standalone:%s, Mode:%s, PedalThreshold:%d, CustomModeAvailable:%s",
